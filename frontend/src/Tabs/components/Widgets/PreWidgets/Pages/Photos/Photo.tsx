...
const Photo = ({photos, onSelectPhoto, setPages, onDeletePhoto}: any) => {
    ...
    const onDelete = () => {
        indexedDBService.deletePhoto(id).then(() => {
          onDeletePhoto()
    ...
    return (
        <div onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onClick={handleClick} ref={photoRef} className={`${css.PhotoContainer} ${showCloseButton ? css.Shake : ""}`}>
            { showCloseButton &&
